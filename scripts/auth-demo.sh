#!/usr/bin/env bash
# Cause P3 (see README) — site-wide basic auth on ISR routes:
...
#
# Requires NUXT_BASIC_AUTH_USER / NUXT_BASIC_AUTH_PASSWORD env vars on the
# deployment (defaults below match the reference deployment).
# Usage: ./scripts/auth-demo.sh https://<deployment>.vercel.app
set -u
B="${1:?usage: ./scripts/auth-demo.sh <deployment-url>}"
CREDS="${CREDS:-repro:repro-password-401}"

hit() { # [-u creds] — prints status/cache/x-vercel-error/content marker
  curl -s -D /tmp/ad.h -o /tmp/ad.b -H 'accept: text/html' "$@" "$B/"
  local st ca er mk
  ...
  mk=$(grep -oE 'site-wide basic auth' /tmp/ad.b | head -1)
  echo "status=$st cache=${ca:--} error=${er:--} ${mk:+CONTENT SERVED}"
}
...
curl -s -X POST -H 'content-type: application/json' \
  -d '{"tags":["page:/"]}' "$B/api/invalidate" >/dev/null
sleep 3
for n in 1 2; do echo "  unauth visit $n: $(hit)"; sleep 2; done
echo
